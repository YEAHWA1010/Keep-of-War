...
UCTargetComponent::UCTargetComponent()
{
	...
	CHelpers::GetAsset<UNiagaraSystem>(&NiagaraAsset, TEXT("NiagaraSystem'/Game/MagicCircleVFX/VFX/NS_TP-Circle_04.NS_TP-Circle_04'"));
}
...
void UCTargetComponent::Toggle()
{
	...
	if (Target != nullptr)
	{
		ACEnemy_AI* EnemyTarget = Cast<ACEnemy_AI>(Target);
		if (EnemyTarget && EnemyTarget->GetCursorWidget())
		{
			UCEnemyLockOnWidget* CursorWidget = Cast<UCEnemyLockOnWidget>(EnemyTarget->GetCursorWidget());
			if (CursorWidget && CursorWidget->GetCursorState())
			{
				CursorWidget->SetTargetCursor(false);
			}
		}
	}
...
void UCTargetComponent::Start()
{
	...
	UKismetSystemLibrary::SphereTraceMultiByProfile(GetWorld(),Location,Location,TraceDistance,"Targeting",false,Ignores,EDrawDebugTrace::None,HitResults,true);
...
void UCTargetComponent::Change(class ACharacter* InCandidate)
{
	...
	if (Target != nullptr)
	{
		ACEnemy_AI* PrevEnemy = Cast<ACEnemy_AI>(Target);
		if (PrevEnemy && PrevEnemy->GetCursorWidget())
		{
			UCEnemyLockOnWidget* PrevCursor = Cast<UCEnemyLockOnWidget>(PrevEnemy->GetCursorWidget());
			if (PrevCursor && PrevCursor->GetCursorState())
			{
				PrevCursor->SetTargetCursor(false);
			}
		}
	}
	
	ACEnemy_AI* NewEnemy = Cast<ACEnemy_AI>(InCandidate);
	if (NewEnemy && NewEnemy->GetCursorWidget())
	{
		UCEnemyLockOnWidget* NewCursor = Cast<UCEnemyLockOnWidget>(NewEnemy->GetCursorWidget());
		if (NewCursor)
		{
			NewCursor->SetTargetCursor(true);
		}
	}
